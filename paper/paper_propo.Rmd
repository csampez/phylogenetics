...
Se desea desarrollar el árbol filogenético de \textit{Saccharomyces cerevisiae} utilizando diferentes algoritmos o métodos para su construcción, basados en caracteres (método de parsimonia), basados en distancias (Neighbor joining, UPGMA, Bayesianos) y máxima verosimilitud (pipeline:SNPhylo).
...
El pipeline está hecho para soportar grandes volúmenes de datos aunque para este análisis, deberemosa evaluar en términos computacionales el número de secuencias para poder crear los árboles filogenéticos a partir los algoritmos que trabajan con distancias y parsimonia.

\begin{itemize}
\item Data Gathering: Obtención de archivos .fasta con información de las bases y mutaciones  
\item Hacer el alineamiento múltiple de las secuencias 
\item A partir del archivo alineado generar la matriz de distancias entre secuencias.
